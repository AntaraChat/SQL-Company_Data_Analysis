/*
Company database schema from "Fundamentals of Database Systems" by Navathe et al.
*/
...
select ssn, dno, salary from employee where salary = (select max(salary) from employee);
select dno, avg(salary) from employee where sex = 'M' group by dno having avg(salary) > 32000;
select ssn, fname from employee e inner join department d on e.dno = d.dnumber where dname = 'Administration';
select ssn, fname, pno from employee e left join works_on w on e.ssn = w.essn;
with male_employees as (select * from employee where sex = 'M') select fname from male_employees;
SET SQL_SAFE_UPDATES=0 ;
create view rich_employee as (select * from employee where salary > 40000);
